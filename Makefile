...
publish:
	...
	docker push $(TARGET)
	if test -z "$$LATEST_TAG"; then docker tag $(TARGET) $(IMAGE_NAME):$(LATEST_TAG); docker push $(IMAGE_NAME):$(LATEST_TAG); fi; \
	if test -z "$$LTS_TAG"; then docker tag $(TARGET) $(IMAGE_NAME):$(LTS_TAG); docker push $(IMAGE_NAME):$(LTS_TAG); fi
